...
/**
 * The actor is the basic entity in the nl.tudelft.simulation.actor package. It
 * implements the behavior of a 'communicating' object, that is able to exchange
 * messages with other actors using devices. The devices can be found in the
 * nl.tudelft.simulation.messaging.device package; the default message object is
 * nl.tudelft.simulation.messaging.Message. The actor has to take care of
 * periodically looking at the devices whether there are any messages, or -in
 * case of some devices- the actor is informed of the fact that there is a
 * waiting message. All this is implemented through the event mechanism. <br>
 * <br>
 * Copyright (c) 2003-2018 Delft University of Technology, Jaffalaan 5, 2628 BX
 * Delft, the Netherlands. All rights reserved. See for project information
 * <a href="https://www.simulation.tudelft.nl/" target=
 * "_blank">www.simulation.tudelft.nl</a>. The source code and binary code of
 * this software is proprietary information of Delft University of Technology.
 * 
 * @author <a href="https://www.tudelft.nl/averbraeck" target="_blank">Alexander
 *         Verbraeck</a>
 */
public abstract class Actor extends InternalActor implements ActorInterface {
	 ...
	 */
	public Actor(final String name, final DEVSSimulatorInterface<Duration> simulator, final Point2d position) {
	...
	/**
	 * adds a receiving device to the actor. The method does not implement a timer
	 * to check periodically for content.
	 * 
	 * @param device         device
	 * @param messageHandler the handler to use
	 */
	public void addReceivingDevice(final ReceivingDeviceInterface device,
			final MessageHandlerInterface messageHandler) {
		this.receivingDevices.put(device, messageHandler);
	}

	/**
	 * adds a receiving device to the actor. The method implements a timer to check
	 * periodically for content.
	 * 
	 * @param device         device
	 * @param messageHandler the handler to use
	...
	/**
	 * The method implements a timer for an existing device to check periodically
	 * for content.
	 * 
	...
	/**
	 * Find the fastest connection between the sender and the receiver, based on the
	 * devices that they share. We look at the minimal transmission delay, and sort
	 * within that on maximum transmission frequency.
	 * 
	...
	protected SendingDeviceInterface resolveFastestDevice(final ActorInterface sender, final ActorInterface receiver) {
		...
		if (possibleDevices.size() == 0) {
			Logger.warn("resolveFastestDevice - No appropriate device(s) found between actor {} and {}",
