...
/**
 * MessageReceiverDelay implements a message queuing mechanism for an actor that
 * handles messages after a (stochastic) delay.
 * <p>
...
public class MessageReceiverDelay extends MessageReceiver {
	...
	/**
	 * the delay distribution that can be changed, e.g., for implementing
	 * administrative delays.
	 */
	private DistContinuousDuration delayDistribution;

	/**
	 * Create a message queuing mechanism for an actor that handles messages after a
	 * (stochastic) delay.
	 * 
	 * @param delayDistribution DistContinuousDuration; the delay distribution for
	 *                          handling messages (note that the distribution can be
	 *                          changed later, e.g., for implementing temporary
	 *                          administrative delays)
	 */
	...
	public <M extends Message> void receiveMessage(final M message, final MessagePolicy<M> messagePolicy) {
		getRole().getActor().getSimulator().scheduleEventRel(this.delayDistribution.draw(), messagePolicy,
				"handleMessage", new Object[] { message });
	}
