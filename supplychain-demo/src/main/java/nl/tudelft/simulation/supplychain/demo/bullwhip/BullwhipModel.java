...
public class BullwhipModel extends SupplyChainModel {
		...
		try {
			if (getSimulator() instanceof AnimatorInterface) {
				...
				new SingleImageRenderable<>(new OrientedPoint3d(0.0, 0.0, -Double.MIN_VALUE), new Bounds3d(800, 600, 0),
						getSimulator(), TestModel.class
								.getResource("/nl/tudelft/simulation/supplychain/demo/bullwhip/images/background.gif"));
			}
			...
			Product casing = new Product(this, "casing", Sku.PIECE, new Money(400.0, MoneyUnit.USD),
					new Mass(10.0, MassUnit.KILOGRAM), new Volume(60.0 * 50.0 * 20.0, VolumeUnit.CUBIC_CENTIMETER),
					0.02);
			...
			Product monitor = new Product(this, "monitor", Sku.PIECE, new Money(200.0, MoneyUnit.USD),
					new Mass(5.0, MassUnit.KILOGRAM), new Volume(60.0 * 40.0 * 10.0, VolumeUnit.CUBIC_CENTIMETER),
					0.01);
			Product pc = new Product(this, "PC", Sku.PIECE, new Money(1100.0, MoneyUnit.USD),
					new Mass(16.0, MassUnit.KILOGRAM), new Volume(10.0 * 5.0 * 4.0, VolumeUnit.CUBIC_CENTIMETER), 0.02);
			...
			// create the bank
			Bank ing = new Bank("ING", "ING", this, new OrientedPoint2d(0, 0),
					"ING", new LeanTradeMessageStore(this.getSimulator()));
			ing.setAnnualInterestRateNeg(0.080);
			ing.setAnnualInterestRatePos(0.025);

			// we create two yellow page 'domains', one between the customers and the
			// retailers,
			...
			DemoYP ypProductionMTS = new DemoYP("YP_production_MTS", getSimulator(), new OrientedPoint3d(600, 40, 0),
					ing);
			DemoYP ypProductionMTO = new DemoYP("YP_production_MTO", getSimulator(), new OrientedPoint3d(700, 40, 0),
					ing);

			// CUSTOMER or MARKET
			DemoMarket customer = new DemoMarket("US East", getSimulator(), new OrientedPoint2d(40, 150), ing,
			...
			DistContinuousDuration generatorStartTime1 = new DistContinuousDuration(new DistConstant(stream, 0.1),
					DurationUnit.HOUR);
			DistContinuousDuration generatorEndTime = new DistContinuousDuration(new DistConstant(stream, 1.0E6),
					DurationUnit.HOUR);
			DistContinuousDuration interBuyingTime1 = new DistContinuousDuration(new DistNormal(stream, 3.0, 1.0),
					DurationUnit.HOUR);
			...
			DistContinuousDuration generatorStartTime2 = new DistContinuousDuration(new DistConstant(stream, 0.1),
					DurationUnit.HOUR);
			DistContinuousDuration interBuyingTime2 = new DistContinuousDuration(new DistNormal(stream, 4.0, 2.0),
					DurationUnit.HOUR);
			...
			DistContinuousDuration generatorStartTime3 = new DistContinuousDuration(new DistConstant(stream, 0.1),
					DurationUnit.HOUR);
			DistContinuousDuration interBuyingTime3 = new DistContinuousDuration(new DistNormal(stream, 5.0, 3.0),
					DurationUnit.HOUR);
			...
			DistContinuousDuration generatorStartTime4 = new DistContinuousDuration(new DistConstant(stream, 504.0),
					DurationUnit.HOUR);
			DistContinuousDuration interBuyingTime4 = new DistContinuousDuration(new DistExponential(stream, 1.0),
					DurationUnit.HOUR);
			...
			// Manufacturers
			DemoManufacturer mtsMan = new DemoManufacturer("MexicoCity_MTS", getSimulator(),
					new OrientedPoint3d(0, -150, 1), ing, new Money(1000000, MoneyUnit.USD), pc, 50, ypCustomerMTS,
					ypProductionMTS, streamMTS, true);
			DemoManufacturer mtoMan = new DemoManufacturer("MexicoCity_MTO", getSimulator(),
					new OrientedPoint3d(0, 150, 1), ing, new Money(1000000, MoneyUnit.USD), pc, 50, ypCustomerMTO,
					ypProductionMTO, streamMTO, false);
	...
	@Override
	public Length calculateDistance(Point<?> loc1, Point<?> loc2) {
		return null;
	}
