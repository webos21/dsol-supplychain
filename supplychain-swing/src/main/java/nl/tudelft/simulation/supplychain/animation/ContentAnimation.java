...
/**
 * This class implements the animation of the content of a message that is sent
 * from one Actor to another Actor. Actually, the content itself does not know
 * when it is being sent (the SendingDevice determines this) or when it will be
 * received (the speed of the communications channel between the devices of the
 * actors determine this). It is even worse: the content itself is unaware of
 * being sent: the Message envelope around the Content is being sent; the
 * Content itself is just the Serializable payload... Therefore, the
 * ContentAnimation subscribes itself on the sending time and receiving time by
 * the sending device.
 * <p>
...
public class ContentAnimation implements Locatable, Serializable {
	...
	/**
	 * Constructs a new animation for any type of content. This constructor will try
	 * to find a default URL of a picture to display.
	 * 
	...
	public ContentAnimation(final TradeMessage content, final Duration delay) {
		this(content, delay,
				content.getClass().getName().substring(content.getClass().getPackage().getName().length() + 1)
						+ ".gif");
	}

	/**
	 * Constructs a new animation for any type of content. This constructor will try
	 * to find a URL of a picture based on a filename.
	 * 
	 * @param content   the Content that should be animated
	 * @param delay     the time to go from sender to receiver
	...
	public ContentAnimation(final TradeMessage content, final Duration delay, final String imageName) {
		this(content, delay, TradeMessage.class.getResource("/supplychain/images/" + imageName));
	}

	/**
	 * Constructs a new animation for any type of content, based on a URL of a
	 * picture.
	 * 
	 * @param content  the Content that should be animated
	 * @param delay    the time to go from sender to receiver
	...
	public ContentAnimation(final TradeMessage content, final Duration delay, final URL imageURL) {
		try {
			...
			this.linearInterpolation = new LinearInterpolation2d(this.simulator.getSimulatorTime().si,
					this.simulator.getSimulatorTime().si + delay.si,
					(OrientedPoint2d) content.getSender().getLocation(),
					(OrientedPoint2d) content.getReceiver().getLocation());
	...
	public OrientedPoint2d getLocation() throws RemoteException {
		return (OrientedPoint2d) this.linearInterpolation.getLocation(this.simulator.getSimulatorTime().si);
	}
