 ...
 */
public class SCRealTimeControlPanel extends DevsControlPanel<Duration, SupplyChainAnimator>
	...
	/**
	 * Generic control panel with a different set of control buttons. The control
	 * panel assumes a RealTimeDevsAnimator and animation, but the model
	 * specification is not necessarily specified as "real time"; its execution is.
	 * 
	 * @param model     DSOLModel&lt;T, ? extends
	 *                  DevsSimulationInterface&lt;T&gt;&gt;; the model for the
	 *                  control panel, to allow a reset of the model
	 * @param simulator S; the simulator. Specified separately, because the model
	 *                  can have been specified with a superclass of the simulator
	 *                  that the ControlPanel actually needs (e.g., model has been
	 *                  specified with a DevsAnimator, whereas the panel needs a
	 *                  RealTimeControlAnimator)
	 * @throws RemoteException when simulator cannot be accessed for listener
	 *                         attachment
	 */
